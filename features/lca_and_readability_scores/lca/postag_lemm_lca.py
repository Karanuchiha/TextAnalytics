...
def get_lemmatized_lines(textlines, lemmatizer):
    lemmatized_lines = list()
    for textline in textlines:
        pos_tagged = nltk.pos_tag(nltk.word_tokenize(textline))  
        lemmatized_sentence = ""
        ...
        lemmatized_lines.append(lemmatized_sentence)
...
def get_lexical_complexity_analysis(text, lemmatizer):
    textlines = text.splitlines()
    lemmatized_lines = get_lemmatized_lines(textlines, lemmatizer)
    ...
    noundict={}
    worddict={}
    wordlistfile=open("lca_and_readability_scores/lca/bnc_all_filtered.txt","r")
    ...
    outputKeys = ("wordtypes", "swordtypes", "lextypes", "slextypes", "wordtokens", "swordtokens", "lextokens", "slextokens", "ld", "ls1", "ls2", "vs1", "vs2", "cvs1", "ndw", "ndwz", "ndwerz", "ndwesz", "ttr", "msttr", "cttr", "rttr", "logttr", "uber", "lv", "vv1", "svv1", "cvv1", "vv2", "nv", "adjv", "advv", "modv")
    # output=filename
    output2 = dict() # output2 = {"filename": filename}
    cnt = 0
    for measure in [len(wordtypes.keys()), len(swordtypes.keys()), len(lextypes.keys()), len(slextypes.keys()), wordtokens, swordtokens, lextokens, slextokens, ld, ls1, ls2, vs1, vs2, cvs1, ndw, ndwz, ndwerz, ndwesz, ttr, msttr, cttr, rttr, logttr, uber, lv, vv1, svv1, cvv1, vv2, nv, adjv, advv, modv]: 
        if type(measure)==type(0.0):
            measure="%.2f" % measure
        # output+=", "+str(measure)
        output2[outputKeys[cnt]] = measure
        cnt += 1


    return output2

def get_lexical_complexity_analysis_sanitized(text, lemmatizer):
    result = get_lexical_complexity_analysis(text, lemmatizer)
    sanitized_result = dict()

    for key, val in result.items():
        if(isinstance(val, str)):
            sanitized_result[key] = float(val)
        else:
